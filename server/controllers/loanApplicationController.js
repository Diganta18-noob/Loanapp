...
const getLoanApplicationsByUserId = async (req, res) => {
    try {
        const applications = await LoanApplication.find({ userId: req.params.userId, isActive: true });
...
const deleteLoanApplication = async (req, res) => {
    try {
        ...
        await createAuditLog({
            ...
            action: 'SOFT_DELETE', entity: 'LoanApplication', entityId: req.params.id,
            details: `Soft-deleted ${application.loanType} application`,
            ipAddress: req.ip,
        });
